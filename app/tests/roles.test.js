...
it('GET /api/roles returns list and respects pagination', async () => {
  const res = await request(app).get('/api/roles?page=1&limit=10&sortBy=periodo&sortOrder=DESC');
...
it('GET /api/roles supports text search (q)', async () => {
  const res = await request(app).get('/api/roles?q=RRHH');
  expect(res.status).toBe(200);
  expect(Array.isArray(res.body.data)).toBe(true);
});

it('GET /api/roles/:id/:periodo returns item with relations', async () => {
  const res = await request(app).get(`/api/roles/${data.rol.id_rol}/${data.rol.periodo}`);
  expect(res.status).toBe(200);
