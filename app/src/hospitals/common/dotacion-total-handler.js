...
module.exports.handleDotacionTotal = async function handleDotacionTotal({ 
  ...
  try {
    ...
    // Parámetros básicos (SIN hospital)
    ...
    // Sin filtro de hospital - solo periodo
    const paramsArr = [periodo]
    const whereClauses = []

    // Filtro opcional de hospital (multi-select desde frontend)
    if (query.hospital_filter) {
      const values = query.hospital_filter.split(',').map(v => v.trim()).filter(Boolean)
      if (values.length > 0) {
        whereClauses.push(`s.sigla IN (?)`)
    ...
    // Calcular distinctValues para CADA campo
