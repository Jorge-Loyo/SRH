// Utilidades compartidas para handlers de tablas (genericTableFull)
...
function getEntityColumns(AppDataSource, Entity, { exclude = [] } = {}) {
  try {
    ...
    const filtered = cols.filter(col => !exclude?.includes(col))
    return filtered
  } catch (e) {
...
async function buildDistinctValues({ repo, alias, columns, req, isNullToken, applyExtraWhere, distinctCol, limit = 500, nullLabel = 'null' }){
  ...
  try {
    ...
    if (typeof applyExtraWhere === 'function') {
      ...
      // applyExtraWhere ya aplica todos los filtros (incluyendo multi-select).
      // No llamar applyMultiWhere para evitar parámetros duplicados en TypeORM.
    } else {
      // Sin applyExtraWhere: aplicar filtros multi-select desde la URL
