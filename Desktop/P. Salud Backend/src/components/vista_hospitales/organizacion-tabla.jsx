...
const OrganizacionTabla = ({ titleOverride, periodo: periodoProp } = {}) => {
  ...
  const kpiStyles = useMemo(() => ({
    activo: {
      ...
      shadow: '0 4px 16px rgba(240, 147, 251, 0.6)'
    },
    comision: {
      background: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)',
      shadow: '0 4px 16px rgba(79, 172, 254, 0.6)'
    },
    retencion: {
      background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
      shadow: '0 4px 16px rgba(102, 126, 234, 0.6)'
    },
    bloqueado: {
      background: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)',
      shadow: '0 4px 16px rgba(250, 112, 154, 0.6)'
    }
  }), [])
    ...
    <Box style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <Box style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <Box style={{
          ...
          <Box style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {/* Botón Seguimiento - Al lado izquierdo de Vista ampliada - Oculto para director */}
            {canSeeRecorridas && (
              <a href={`/admin/pages/RecorridasDetalle?hospital=${hospital}`} style={{ textDecoration: 'none' }}>
                ...
                >
                  <Icon icon="FileText" style={{ marginRight: 4 }} />
                  Seguimiento
                </Button>
              </a>
            )}
            ...
            >
              <Icon icon="Maximize" style={{ marginRight: 4 }} />
              Vista ampliada
            </Button>
          </Box>
        </Box>
