...
function createJWTAuthMiddleware(admin, usePublic) {
  ...
  return async (req, res, next) => {
    ...
    // 5. IDLE TIMEOUT: Auto-refresh JWT if expiration is near
    // If JWT expires in less than 5 minutes, generate a new one
    try {
      const jwt = require('jsonwebtoken');
      ...
      if (decoded?.exp) {
        ...
        const refreshThresholdSeconds = 5 * 60; // Refresh if less than 5 minutes left
